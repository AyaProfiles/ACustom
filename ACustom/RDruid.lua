...
local function zPXTTg()end;local seMLr,qX,h_8,xL7OTb,w8T3f,KqL,vfIyB;local quNsijN,QUh2tc,qboV,nSBOx7;local u,Q,Ki1,zz1QI;local kFTAh,LBf,dijn4Ph,CO1,RlZo,SUn,Ib4,fjV1G2
...
missedspell=currentspell end end end,"UI_ERROR_MESSAGE","UNIT_SPELLCAST_SENT")local function QYf1(pVRj)if pVRj then return 1 else return 0 end end;local function RfsnisO(pVRj)
return pVRj~=0 end
local function RfsnisO()
...
quNsijN=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"WildGrowthHP")
QUh2tc=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"WildGrowthNum")
qboV=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"EfflorescenceHP")
nSBOx7=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"EfflorescenceNum")
KqL=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"IronbarkHP")
vfIyB=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"SwiftmendHP")
Ki1=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"RenewalHP")
Q=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"ConvokeHP")
...
if hDc_M.getn(IN)>0 and QKKks_zt~=nil then
for pVRj=1,hDc_M.getn(IN)do for pVRj in
JLCOx_ak.gmatch(IN[pVRj],"([^%s]+)")do
...
wqU76o("party"..QKKks_zt)+ (PrPyxMK("party"..QKKks_zt)or 0)-hDc_M("party"..QKKks_zt)local QKKks_zt=tczrIB("party"..QKKks_zt)if
...
(PrPyxMK("raid"..QKKks_zt)or 0)-hDc_M("raid"..QKKks_zt)local QKKks_zt=tczrIB("raid"..QKKks_zt)if
Vu0cCAf/QKKks_zt<=fuZ3z86 then pVRj=pVRj+1 end end end end
if mP3mlD()==0 then
local QKKks_zt=
wqU76o("player")+ (PrPyxMK("player")or 0)-hDc_M("player")local Vu0cCAf=tczrIB("player")
if QKKks_zt/Vu0cCAf<=fuZ3z86 then pVRj=pVRj+1 end end;return pVRj end
...
local function XL_()RfsnisO()
if qkP7O5()then
urkh=QKKks_zt:GetEnemiesInRangeFilter(10)t5jzEd9=WYdR:GetEnemiesInSplashRange(10)zhzpBSx=#urkh
TjhsnP=WYdR:GetEnemiesInSplashRangeCount(10)else zhzpBSx=1;TjhsnP=1 end;if QKKks_zt:StealthUp()then return end;local zhzpBSx=dijn4Ph()if zhzpBSx then
return zhzpBSx end
if
DI and Are7xU:IsAPlayer()and Are7xU:IsDeadOrGhost()and not Are7xU:IsEnemy()and
Are7xU:IsInParty()then
if
pVRj.Revive:IsReady(QKKks_zt)then if Vu0cCAf(pVRj.Revive)then return'Resurrection'end end end;if pVRj.Barkskin:IsReady()and
QKKks_zt:HealthPercentage()<u then
if Vu0cCAf(pVRj.Barkskin)then return'Barkskin Defensive'end end
if

pVRj.Renewal:IsReady()and QKKks_zt:HealthPercentage()<Ki1 then if Vu0cCAf(pVRj.Renewal)then return'Renewal Deffensive'end end
if pVRj.Innervate:IsReady()and DI then
...
return
pVRj:HealthPercentage()<KqL and not LB1Z(pVRj)end)then return'Ironbark Members'end end
if
...
(er.TargetIsValid()or DI)and not QKKks_zt:IsMoving()and hDc_M()<2 and
IN(qboV)>=nSBOx7 and not b and
...
QKKks_zt:HealthPercentage()<vfIyB and
...
(IN(quNsijN)>=QUh2tc or QKKks_zt:BuffUp(pVRj.IncarnationTreeofLifeBuff))and QKKks_zt:BuffDown(pVRj.SouloftheForestBuff)then
if
...
pVRj.Rejuvenation:IsCastable()and(IN(quNsijN)>=QUh2tc or
...
(
IN(quNsijN)>=QUh2tc or QKKks_zt:BuffUp(pVRj.IncarnationTreeofLifeBuff))then
...
"Treant Form OOC"end end end end;local function pVRj()N9L()return XL_()end
