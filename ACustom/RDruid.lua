...
local wqU76o=UnitHealth;local LB1Z=GetUnitName;local N9L=UnitGUID;local hDc_M=table;local qW0lRiD1=GetTime;local Q=tonumber
...
local t5jzEd9,rHSjalVy,JZAU2,zPXTTg,seMLr,qX,h_8;local xL7OTb,w8T3f,KqL,vfIyB;local quNsijN,QUh2tc,qboV,nSBOx7;local u,Ki1,zz1QI,kFTAh;local LBf,dijn4Ph,CO1,RlZo,SUn,Ib4,fjV1G2;local Do;local Ch={}
...
missedunit=currentspelltarget;missedspell=currentspell end end end,"UI_ERROR_MESSAGE","UNIT_SPELLCAST_SENT")local function _TqYJ4(pVRj)if pVRj then return 1 else return 0 end end;local function DI(pVRj)
return pVRj~=0 end
local function DI()
kFTAh=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"DPSThreshold1")
...
KqL=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"EfflorescenceHP")
vfIyB=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"EfflorescenceNum")
qX=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"IronbarkHP")
h_8=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"SwiftmendHP")
zz1QI=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"RenewalHP")
Ki1=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"ConvokeHP")
...
LBf=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"ConvokeOption")
quNsijN=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"TopTrinketTarget")
QUh2tc=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"TopTrinketOption")
qboV=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"BotTrinketTarget")
nSBOx7=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"BotTrinketOption")
u=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"BarkSkinHP")
dijn4Ph=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"FormSwap")
CO1=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"FormSwapRange")if CO1 and CO1 <=5 then CO1=5 end
fjV1G2=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"AutoProwl")
RlZo=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"ManaManagement")Do=Are7xU:IsInMeleeRange(CO1)
...
Ib4=fuZ3z86.Config.GetSetting('Aya_Rdruid_Config',"OOCForm")end
local function CO1(QKKks_zt,fuZ3z86)local Vu0cCAf,pVRj
...
local function Ki1()if missedunit then
hDc_M.insert(Ch,(CO1(missedunit).." "..qW0lRiD1()))missedunit=nil end
if
hDc_M.getn(Ch)>0 then
for QKKks_zt=1,hDc_M.getn(Ch)do
if Ch[QKKks_zt]~=nil then
for pVRj in
iD1IUx.gmatch(Ch[QKKks_zt],"([^%s]+)")do pVRj=Q(pVRj)if R1FIoQI(pVRj)=="number"then if pVRj<=qW0lRiD1()-1.5 then
hDc_M.remove(Ch,QKKks_zt)end end end end end end end
local function LB1Z(QKKks_zt)
if hDc_M.getn(Ch)>0 and QKKks_zt~=nil then
for pVRj=1,hDc_M.getn(Ch)do for pVRj in
iD1IUx.gmatch(Ch[pVRj],"([^%s]+)")do
...
if fuZ3z86 ==nil or R1FIoQI(fuZ3z86)=="string"then return nil end;fuZ3z86=fuZ3z86/100;local QKKks_zt=0
...
local function N9L()
for QKKks_zt=1,4 do local Are7xU,fuZ3z86,QKKks_zt,Vu0cCAf=NsoTwDs(QKKks_zt)
if fuZ3z86 ==
pVRj.Efflorescence:Name()then return
(HGli(QKKks_zt+Vu0cCAf-qW0lRiD1()+.5))or 0 end end;return 0 end
local function CO1()
if


pVRj.ConvoketheSpirits:IsReady()and Are7xU:IsInMeleeRange(40)and LBf['ConvokeDamage']and(lqT("boss1")or
XL_.FightRemains(zhzpBSx)>30)and XL_.CombatTime()>5 then
...
if
dijn4Ph and pVRj.CatForm:IsReady(QKKks_zt)and
...
if Hv<=3 and Hv>0 and dijn4Ph then
...
if Vu0cCAf(pVRj.Moonfire)then return'Moonfire - DPS (1-3 Targets)'end end elseif Hv>=4 and dijn4Ph then
if Do then
...
if(not Do or not dijn4Ph)then
if

pVRj.MoonkinForm:IsReady(QKKks_zt)and QKKks_zt:BuffDown(pVRj.MoonkinForm)and dijn4Ph then if Vu0cCAf(pVRj.MoonkinForm)then return
...
local function dijn4Ph()local ZG,dijn4Ph,Ch,dijn4Ph,dijn4Ph,dijn4Ph=DFb100j:Update()DI()
if qkP7O5()then
...
Vu0cCAf(pVRj.Revive)then return'Resurrection'end end end;if pVRj.Barkskin:IsReady()and
QKKks_zt:HealthPercentage()<u then
if Vu0cCAf(pVRj.Barkskin)then return'Barkskin Defensive'end end
if

pVRj.Renewal:IsReady()and QKKks_zt:HealthPercentage()<zz1QI then if Vu0cCAf(pVRj.Renewal)then return'Renewal Deffensive'end end
if
pVRj.Innervate:IsReady()and QKKks_zt:AffectingCombat()then
if SUn["InnervatePlayer"]then
if
fuZ3z86.CastCycleAlly(pVRj.Innervate,Ch,function(pVRj)
return

pVRj:GUID()==QKKks_zt:GUID()and pVRj:ManaPercentage()<=InnervateMana and not LB1Z(pVRj)end)then return'Innervate Player'end end
...
not QKKks_zt:IsMoving()and LBf['ConvokeHeal']then if pVRj.NaturesSwiftness:IsReady()then
if
...
(
DFb100j:BuffTotal(pVRj.LifebloomResto)+_TqYJ4(QKKks_zt:BuffUp(pVRj.LifebloomResto)))<2 and
((
...
_TqYJ4(QKKks_zt:BuffUp(pVRj.LifebloomResto)))<2)or
(not pVRj.Undergrowth:IsAvailable()and
(
DFb100j:BuffTotal(pVRj.LifebloomResto2)<
1+_TqYJ4(QKKks_zt:BuffUp(pVRj.LifebloomResto2)))))and
((
...
(
DFb100j:BuffTotal(pVRj.LifebloomResto2)+_TqYJ4(QKKks_zt:BuffUp(pVRj.LifebloomResto2)))<1 and not pVRj.Undergrowth:IsAvailable()and
((
...
hDc_M(KqL)>=vfIyB and not XL_ and
((QKKks_zt:BuffDown(pVRj.CatForm)and QKKks_zt:BuffDown(pVRj.BearForm)))then
if Vu0cCAf(pVRj.Efflorescence)then return"Efflorescence Members"end end;if QKKks_zt:AffectingCombat()and hDc_M(kFTAh)==0 and
er.TargetIsValid()and not
fuZ3z86.Toggle:GetToggle("StopDPS")then return
CO1()end
if

pVRj.Swiftmend:CooldownRemains()<=0 and not QKKks_zt:IsMoving()then
if
...
QKKks_zt:BuffUp(pVRj.Rejuvenation))and not
...
kFTAh<100 and
not fuZ3z86.Toggle:GetToggle("StopDPS")then return CO1()end
if
...
if
not QKKks_zt:AffectingCombat()and Ib4 ~="NoneForm"then
if pVRj.BearForm:IsCastable()and Ib4 =="BearForm"and
QKKks_zt:BuffDown(pVRj.BearForm)then if Vu0cCAf(pVRj.BearForm)then
return"Bear Form OOC"end elseif

pVRj.CatForm:IsCastable()and Ib4 =="CatForm"and QKKks_zt:BuffDown(pVRj.CatForm)and not fjV1G2 then if
Vu0cCAf(pVRj.CatForm)then return"Cat Form OOC"end elseif
pVRj.TravelForm:IsCastable()and Ib4 =="TravelForm"and
(
...
"Treant Form OOC"end end end end;local function pVRj()Ki1()return dijn4Ph()end
